...
const getTriggerKeydownHandler = (
  openAction: () => void,
  closeAction: () => void,
    ...
    switch (e.key) {
      ...
      case "Tab":
        if (e.shiftKey) {
          ...
          /** ACT AS DOWN ARROW */
          /** Focus on next item or the first item */
          setActiveIndex((prev) => (prev + 1 === optionsLength ? 0 : prev + 1));
          break;
        }
    }
  };
...
const useAccessibleMultiSelect = ({
  ...
  useEffect(() => {
    ...
    if (isFocus) {
      keyDownCallback = getTriggerKeydownHandler(
        openAction,
        closeAction,
        setActiveIndex
      );
      ...
      keyDownCallback = getItemKeydownHandler(
        closeAction,
        activeIndex,
        optionsLength,
        setActiveIndex,
